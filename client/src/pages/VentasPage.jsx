...
import { ShoppingCart, Search, Plus, Minus, Trash2, Check, User, Users, X, UserPlus, Phone, MapPin, Loader2, ArrowRight } from 'lucide-react';
...
const VentasPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('productos'); // 'productos' | 'carrito'
  ...
  const addToCart = (product) => {
    ...
    if (!isMobile) {
      // Small visual feedback or something on desktop
    }
  };
          ...
          <div className={`bg-white dark:bg-[#1C1C1E] md:rounded-[32px] shadow-shadow-lg border border-border overflow-hidden flex flex-col ${
            ...
            <div className={`p-6 bg-bg-primary/50 dark:bg-bg-dark border-t border-border space-y-4 ${isMobile ? 'pb-10' : ''}`}>
              <div className="flex justify-between items-center text-text-secondary font-medium">
                ...
                <span className="font-mono">S/ {calculateTotal().toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-text-primary text-2xl font-black tracking-tighter">
                <span>Total</span>
                <span className="text-brand-primary font-mono">S/ {calculateTotal().toFixed(2)}</span>
              </div>
              <button 
      ...
      {isMobile && activeTab === 'productos' && cart.length > 0 && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold">S/ {calculateTotal().toFixed(2)}</span>
            <ArrowRight size={20} />
          </div>
        </button>
      )}

      {/* Client Modal adapted for mobile (Bottom Sheet) */}
      {isClientModalOpen && (
